...
{
    [TestClass]
    public class Post_Usuario_Cenario2_BadRequest : BaseBDDTestFixture
    {
        ...
        public void Quando_CadastroNovoUsuario_SemPreencherCampoConfirmacaoSenhaCorretamente_Entao_Erro()
        {
            // Assert
            Assert.IsNotNull(post);
            Assert.AreEqual(HttpStatusCode.BadRequest, post.StatusCode);
            Assert.IsTrue(retorno.Contains("Campos Senha e Confirmação de Senha não conferem."));
        }
    }
}
