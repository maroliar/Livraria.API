...
{
    [TestClass]
    public class Post_Livro_Cenario1_BadRequest : BaseBDDTestFixture
    {
        ...
        public void Quando_CadastroNovoLivro_SemPreencherCamposCorretamente_Entao_Erro()
        {
            // Assert
            Assert.IsNotNull(post);
            Assert.AreEqual(HttpStatusCode.BadRequest, post.StatusCode);
            Assert.IsTrue(retorno.Contains("Campo Nome do Livro é obrigatório."));
            Assert.IsTrue(retorno.Contains("Campo Autor é obrigatório."));
        }
    }
}
